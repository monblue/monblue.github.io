      ...
      TediousRequest.prototype.query = function(command, callback) {
        ...
        return this._acquire((function(_this) {
          return function(err, connection) {
            ...
            if (!err) {
              if (_this.verbose) {
                _this.doLog("---------- sql query ----------\n    query: " + command);
              }
              if (_this.canceled) {
                if (_this.verbose) {
                  _this.doLog("---------- canceling ----------");
                }
                ...
                if (_this.verbose) {
                  if (errors.length) {
                    for (_i = 0, _len = errors.length; _i < _len; _i++) {
                      error = errors[_i];
                      _this.doLog("    error: " + error);
                    }
                  }
                  elapsed = Date.now() - started;
                  _this.doLog(" duration: " + elapsed + "ms");
                  _this.doLog("---------- completed ----------");
                }
                if (recordset) {
              ...
              req.on('returnValue', function(parameterName, value, metadata) {
                if (_this.verbose) {
                  if (value === tds.TYPES.Null) {
                    _this.doLog("   output: @" + parameterName + ", null");
                  } else {
                    _this.doLog("   output: @" + parameterName + ", " + (_this.parameters[parameterName].type.declaration.toLowerCase()) + ", " + value);
                  }
                }
              ...
              for (name in _ref) {
                ...
                if (_this.verbose) {
                  if (param.value === tds.TYPES.Null) {
                    _this.doLog("    input: @" + param.name + ", null");
                  } else {
                    _this.doLog("    input: @" + param.name + ", " + (param.type.declaration.toLowerCase()) + ", " + param.value);
                  }
                }
            ...
            if (!err) {
              ...
              req = new tds.Request(procedure, function(err) {
                ...
                if (_this.verbose) {
                  ...
                  elapsed = Date.now() - started;
                  _this.doLog("   return: " + returnValue);
                  _this.doLog(" duration: " + elapsed + "ms");
                  _this.doLog("---------- completed ----------");
                }
                _this._cancel = null;
              ...
              for (name in _ref) {
                ...
                if (_this.verbose) {
                  if (param.value === tds.TYPES.Null) {
                    _this.doLog("   " + (param.io === 1 ? " input" : "output") + ": @" + param.name + ", null");
                  } else {
                    _this.doLog("   " + (param.io === 1 ? " input" : "output") + ": @" + param.name + ", " + (param.type.declaration.toLowerCase()) + ", " + param.value);
                  }
                }
                if (param.io === 1) {
                  req.addParameter(param.name, getTediousType(param.type), parameterCorrection(param.value), {
