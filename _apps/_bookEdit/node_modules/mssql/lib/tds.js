    ...
    TDSRequest = (function(_super) {
        ...
        if (command.length === 0) {
          return process.nextTick(function() {
            ...
            if (this.verbose && !this.nested) {
              ...
              this.doLog(" duration: " + elapsed + "ms");
              this.doLog("---------- completed ----------");
            }
          ...
          return function(err, connection) {
            ...
            if (!err) {
              ...
              req.on('done', function(res) {
                var elapsed, error, last, _i, _len, _ref2, _ref3;
                if (!_this.nested) {
                  if (recordset) {
                    _this.emit('recordset', recordset);
                  }
                  if (handleOutput) {
                    ...
                    for (name in _ref3) {
                      ...
                      if (_this.verbose) {
                        _this.doLog("   output: @" + param.name + ", " + param.type.declaration + ", " + param.value);
                      }
                    }
                  }
                  if (_this.verbose) {
      ...
      TDSRequest.prototype.execute = function(procedure, callback) {
        ...
        for (name in _ref1) {
          param = _ref1[name];
          if (this.verbose) {
            this.doLog("   " + (param.io === 1 ? " input" : "output") + ": @" + param.name + ", " + param.type.declaration + ", " + param.value);
          }
          if (param.io === 2) {
